...
\scalebox{0.8}{\begin{tabular}{|l|l|l|l|l|l|}
\hline
...
\multirow{2}{*}{$\encT$}   &  \multirow{2}{*}{$\encT'$}   & Generates a new attribute  $A'$ (in one-hot-coding) to represent\\ & & & & & the data for both the attributes $A_i$ and $A_j$   \\ \cline{2-6}
                                ...
                                & \textsf{GroupByCount}              & $\groupbystar_{A}(\cdot)$    &$\encT$       & $\tilde{\encV}$        &  Returns encrypted table of $A$ and count in one-hot-encoding    \\ \cline{2-6}
...
\multirow{2}{*}{Measurement}    & \textsf{Laplace}     & $\lap_{\epsilon,\Delta}(\cdot)$    &  $\encV$      &   $\hat{V}$    &  Adds Laplace noise to $\encV$    \\ \cline{2-6}
                                & \textsf{NoisyMax}     & $\noisymax_{\epsilon,\Delta}(\cdot)$         & $\encV$       &  $\hat{\mathcal{P}}$      & Adds Laplace noise to $\encV$ and 
                                returns indices of the top noisy value             \\ \hline
\end{tabular}
}
\end{table*}
