...
\multirow{7}{*}{Transformation} & CrossProduct &  $\crossproduct_{A_i,A_j}(\cdot)$ &  $\encT$   &  $\encT'$   & Generates new attribute $A'$ (in one-hot-encoding) to represent $A_i$ and $A_j$  \\ \cline{2-6}
                                & Project     & $\project_{A^*}(\cdot)$  &  $\encT$       &  $\encT'$       &  Discards all attributes but $A*$ \\ \cline{2-6}
                                & Filter       & $\filter_{\phi}(\cdot)$   &  $\encT$      &   $\encB'$     &  Zeros out records not satisfying $\phi$    in $\encB$          \\ \cline{2-6}
                                & Count              & $\countagg(\cdot)$         & $\encT$  &  $\encC$    &  Counts the number of 1s in $\encB$               \\ \cline{2-6}
