...
\begin{table*}
\caption {\system Primitives}\label{tab:primitives} \vspace{-3mm}
\scalebox{0.7}{
\begin{tabular}{|l|l|l|l|l|l|}
\hline
\bf{Types}                           & \bf{Name}         & \bf{Notation} & \bf{Input} & \bf{Output} & \bf{Functionality} \\ \hline \hline
\multirow{7}{*}{Transformation} & \textsf{CrossProduct} &  $\crossproduct_{A_i,A_j\rightarrow A'}(\cdot)$ &
$\encT$   &  $\encT'$   & Generates a new attribute  $A'$ (in one-hot-coding) to represent the data for both the attributes $A_i$ and $A_j$   \\ \cline{2-6}
...
Post-processing      &\textsf{PostOrder}              &  $\postorder_{\leq}(\cdot)$  & $\hat{V}$  & $\hat{V}'$ & Post-process the plain text vector such that $V[i] \leq V[j]$ for $i<j$ \\ \hline
\end{tabular}
}
\end{table*}
