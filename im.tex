...
\stitle{\textsf{Filter}} $\filter_{\phi}(\cdot)$: Let $\phi$ be a predicate on a single attribute of the form $r.A \in V_A$. Row $i$ satisfies the filter if one of the bits corresponding to positions in $V_A$ are set to 1. Thus the filter bit corresponding to row $i$ is set to: $\encB[i] = labMult(\encB[i], \bigoplus_{l\in V_{A_j}}\tilde{\bf{v}}_j[l])$.
