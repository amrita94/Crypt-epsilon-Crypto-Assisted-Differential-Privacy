\vspace{-0.35cm}\subsection{Program Examples}
...
\item  The sequence of transformation operators for each such query is $count(\sigma(\pi(\encD))$. Thus the sensitivity of each range query is upper bounded by the product of the stabilities of these three operators which is $1$. \item Thus the input to the measurement operator $Lap$ is privacy parameter $\frac{\epsilon}{100}$ and sensitivity 1. \item Finally, the program performs post processing over the noisy plaintext output $\hat{V}=[\hat{c}_1,...,\hat{c}_{100}]$. We denote this operator by $post_{c.d.f}(\hat{V})$. This operator input a noisy histogram $\hat{V}$
