...
\system supports all programs that can be expressed as a sequence of \system primitives followed by arbitrary post-processing. Consider a database schema $\langle Age$, $Gender$, $NativeCountry$, $Department$, $Salary\rangle$. We show several \system program examples in Table~\ref{tab:programexamples} over this database. In this table, P2 showcases a post processing step over the noisy plaintext output. We denote this operator by $post_{c.d.f}(\hat{V} )$. This operator input a noisy histogram $\hat{V}$
