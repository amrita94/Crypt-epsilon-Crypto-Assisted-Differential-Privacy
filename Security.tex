...
\textbf{Output}:- Differentially private output for Q, $\hat{c}$\\\\\textbf{Phase 1:} \textsf{AS}, \textsf{CSP} and $\textsf{DO}_i$ jointly run protocol $\Pi_1$\\\textbf{Phase 2:} \textsf{AS} and \textsf{CSP} run protocol $\Pi_2$ to generate $\hat{c}$ }} \label{ideal} \caption{Ideal functionality $\mathcal{F}$ of Crypt$\epsilon$} \end{figure}
