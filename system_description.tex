...
Let $P$ be a program with sensitivity $\Delta$ that is submitted by the external data analyst to protocol $\Pi$ and it is to be run on the data set $\mathcal{D}$ with privacy parameter $\epsilon$. $output^{\Pi}$ is the output of \system that is visible to the data analyst. Thus we define the ideal functionality $\mathcal{F}=(\mathcal{F}_1,\mathcal{F}_2)$ of the protocol $\Pi$ as follows.
\\If $P$ uses Laplace mechanism \cite{Dork}, then
\begin{gather} 
\mathcal{F}_1(P,\mathcal{D},\Delta,\epsilon)=P(\mathcal{D})+\eta_2\\\mathcal{F}_2(P,\mathcal{D},\Delta,\epsilon)=P(\mathcal{D})+\eta_1\\output^{\Pi}=P(\mathcal{D})+\eta_1+\eta_2\\\eta_1,\eta_2 \in Lap(\frac{\Delta}{\epsilon})\end{gather}
If $P$ uses NoisyMax mechanism \cite{Dork}, then
\begin{multline}
\mathcal{F}_1(P,\mathcal{D},\Delta,\epsilon)=\mathcal{F}_2(P,\mathcal{D},\Delta,\epsilon)=output^{\Pi}\\=NoisyMax(P(\mathcal{D}),\Delta,\epsilon)
\end{multline} where $P(\mathcal{D})$ produces a vector. Note that $output^{\Pi}$ in eq 5 is $\epsilon-DP$ by Theorem 2.
...
$S$ if there exists  P.P.T algorithms $Sim_1$ and $Sim_2$ such that \begin{gather} \big\{Sim_1({D_i, i \in S},\mathcal{F}_1(P,\mathcal{D},\Delta,\epsilon)),\mathcal{F}(P,\mathcal{D},\Delta,\epsilon)\big\} \stackrel{C}{\equiv} \big\{view^{\Pi}_{Adv1},output^{\Pi}\big\} \label{adv1}
\\\big\{Sim_2({D_i, i \in S},\mathcal{F}_2(P,\mathcal{D},\Delta,\epsilon)),\mathcal{F}(P,\mathcal{D},\Delta,\epsilon))\big\} \stackrel{C}{\equiv}\big\{ view^{\Pi}_{Adv2},output^{\Pi}\big\}\label{adv2}
