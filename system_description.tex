...
Guided by the above principles we design a two-server model for \system as described in the following sections. %We discuss about some additional \system architectural choices in section ~\ref{architecture}.

\subsection{Crypt$\epsilon$ Components}
\system comprises of the following parties:\\(1)\textbf{ Data-Owners (\textsf{DO})}-  Each data-owner $\textsf{DO}_i, i \in [m]$ has  a
...
Here we present a brief security sketch of  Crypt$\epsilon$. The formal security analysis is presented in the  Appendix B.  \\
(1) The \textsf{AS} has access to only the encrypted records and does not have knowledge of the secret key, $sk$. Thus every measurement on the data needs an interaction with the \textsf{CSP} for decryption. Recall that in Crypt$\epsilon$ both the \textsf{AS} and the \textsf{CSP} are responsible for the noise addition.  Hence, the \textsf{CSP} always adds noise of its own before releasing the decrypted data back to the \textsf{AS}. This ensures that the \textsf{AS} is only able to take noisy measurements of the data.  Moreover, in addition to the \textit{Program Executor} module of the \textsf{AS}, the data analyst communicates the Crypt$\epsilon$ program and the privacy parameter $\epsilon$ to the \textit{Privacy Engine} module of the \textsf{CSP} as well. This enables the \textsf{CSP} to compute the sensitivity  of the program for itself (required while adding its own noise to the data) and thwart any privacy budget over expenditure attempts, in the semi-honest model. \\
(2) The encrypted database is stored in the \textsf{AS} with the \textsf{CSP} having no access to it. The interactions between the \textsf{CSP} and the \textsf{AS} can be categorized as   \\$\bullet$ Yao's garbled circuits \\$\bullet $ decryption of ciphers for differentially private results \\$\bullet$ decryption of ciphers for masked data  \\ The garbled circuit interactions are secure by the semantic security of secure computations and the random masks in the third type of interactions hides the true plaintext  from the \textsf{CSP}. Thus even the \textsf{CSP} is just allowed only a differentially private view of the database.\\
(3) The external data analyst who submits \system programs to the \textsf{AS} and the \textsf{CSP}, only receives the differentially private program output and nothing else. 
